...
message at time_stamp and that confirmation message was either confirmed ('confirmed') or expired
without confirming ('timeout').
...
SELECT s.user_id,
    ROUND(
        IFNULL(SUM(CASE WHEN c.action = 'confirmed' THEN 1 ELSE 0 END) * 1.0 / COUNT(c.user_id), 0),
        2
